#!/usr/bin/env node
/**
 * Seed script for Inkwell Scanner backend.
 * Reads from ML service's cards_db.json if available,
 * otherwise uses built-in seed data.
 *
...
// Check if we should import from ML service
const ML_CARDS_DB = path.join(__dirname, '..', '..', 'ml-service', 'cards_db.json');

if (fs.existsSync(ML_CARDS_DB)) {
  console.log('ML card database found. Running import...\n');
  require('./import-ml-cards');
  process.exit(0);
}

console.log('ML card database not found. Using built-in seed data.\n');

// ============================================================
// BUILT-IN SEED DATA (fallback when ML service isn't available)
// ============================================================
...
console.log('Seeding Lorcana card database...\n');

const db = getDb();

// Check if cards already exist
const count = db.prepare('SELECT COUNT(*) as count FROM cards').get();
const setCount = db.prepare('SELECT COUNT(*) as count FROM set_metadata').get();

if (count.count > 0) {
  console.log(`Database already has ${count.count} cards and ${setCount.count} sets.`);
  console.log('To re-seed: run DELETE FROM cards; DELETE FROM set_metadata; then re-run this script.');
  closeDb();
  process.exit(0);
}
...
console.log(`Inserted ${SET_METADATA.length} set metadata entries`);
...
const transaction = db.transaction((cards) => {
  for (const card of cards) {
    ...
    insertCard.run(uuidv4(), card.card_name, card.set_name, card.set_code, card.set_number,
...
const updateSetCount = db.prepare(`
...
const newCount = db.prepare('SELECT COUNT(*) as count FROM cards').get();
console.log(`\nSeeded ${CARD_DATA.length} cards. Total cards: ${newCount.count}`);

// Show sets
const sets = db.prepare('SELECT name, code, release_date, card_count, total_cards FROM set_metadata ORDER BY release_date').all();
console.log('\nSets in database:');
for (const set of sets) {
  console.log(`  ${set.code} - ${set.name}: ${set.card_count}/${set.total_cards} cards (released ${set.release_date})`);
}
